...
```bash
$ yarn add --dev eslint
$ yarn add --dev @munu/eslint-config-bases:"workspace:^"
```
...
module.exports = {
  ...
  extends: [
    ...
    "@munu/eslint-config-bases/playwright",
...
> **Tip:** "@munu/eslint-config-bases/prettier" must be set at the end to disable any
> conflicting rules.
...
To prevent conflicts between prettier and eslint, you must re-export the prettier base from `@munu/eslint-config-bases`.
